...
class Builder {
  ...
  exec() {
    return this._client.createExecutor(this).exec()
  }

  _formatValues(values) {
    const isExplicit = (x) => x.hasOwnProperty('col') && x.hasOwnProperty('val') // FIXME: check if explicit format 
    const formatVal = (x) => isExplicit(x)
      ? x
      : Object.entries(x).map(([col, val]) => ({col, val})) 

    const res = Array.isArray(values)
      ? values.map(formatVal)
      : [formatVal(values)]
    return Array.isArray(res[0]) ? res : [res] // FIXME: 
  }
