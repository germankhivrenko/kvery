...
class Pool extends EventEmitter {
  ...
  async seize() {
    ...
    const connection = await this._client.createConnection(this._options.connection)
  ...
  async end() {
    ...
    await Promise.all(this._pool.map(async (pc, index) => {
      this._free[index] = true
  ...
  async query(q) {
    ...
    const poolConn = await this.seize()
    const res = await poolConn.query(q)
    poolConn.release()
  ...
  _validateCtorParams(client, options) {
    const {Client} = require('./client') // FIXME:
